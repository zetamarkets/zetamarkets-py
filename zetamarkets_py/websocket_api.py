...
class SolanaWsClientProtocol(WebSocketClientProtocol):
    ...
    async def transaction_subscribe(
        ...
        # req = LogsSubscribe(filter_, config, req_id)
        mentions = []
        if isinstance(filter_, RpcTransactionLogsFilterMentions):
            mentions.append(str(filter_.pubkey))
        req = request(
        ...
        # await self.send_data(req)
        self.sent_subscriptions[req_id] = req
        await super().send(json.dumps(req))

    async def transaction_unsubscribe(
        self,
        subscription: int,
    ) -> None:
